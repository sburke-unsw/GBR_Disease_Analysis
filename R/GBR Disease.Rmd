...
CombinedData_TempVars <- readRDS(here("Rdata", "CombinedData_TempVars v5.RDS"))
...
ggplot(gbr2) +
  geom_jitter(aes(x = Report_Year, y = Disease_Num, col = Site_ID)) +
  ylim(0.5, 5.5) +
  theme(legend.position = "none")

ggplot(gbr2) +
  geom_histogram(aes(x = Disease_Num)) +
...
ggplot(gbr2) +
  geom_col(aes(x = Report_Year, y = Disease_Num)) +
...
ws_p <- ggplot(gbr) +
  geom_col(aes(x = REPORT_YEAR, y = (WHITESYNDROME/sum(WHITESYNDROME, na.rm = TRUE))), na.rm = TRUE) +   # not correct? Need the sum of WS from that year, not whole dataset
  labs(x = "Year", y = "White Syndrome") +
  theme_bw()
# White syndrome persistent in high abundance in GBR for all years examined

seb_p <- ggplot(gbr) +
  geom_col(aes(x = REPORT_YEAR, y = (SEB_DISEASE/sum(SEB_DISEASE, na.rm = TRUE))), na.rm = TRUE) +
  labs(x = "Year", y = "Skeletal Eroding Band") +
  theme_bw()
# SEB rarer in early years, but once appeared, persisted in GBR

br_p <- ggplot(gbr) +
  geom_col(aes(x = REPORT_YEAR, y = (BROWNB_DISEASE/sum(BROWNB_DISEASE, na.rm = TRUE))), na.rm = TRUE) +
  labs(x = "Year", y = "Brown Band") +
  theme_bw()
# some small incidence 2004-05, really took off 2006 and on

bl_p <- ggplot(gbr) +
  geom_col(aes(x = REPORT_YEAR, y = (BLACKBAND/sum(BLACKBAND, na.rm = TRUE))), na.rm = TRUE) +
  labs(x = "Year", y = "Black Band") +
  theme_bw()
# present throughout all years examined. Intense breakout recorded in many transects in 2003

an_p <- ggplot(gbr) +
  geom_col(aes(x = REPORT_YEAR, y = (AN/sum(AN, na.rm = TRUE))), na.rm = TRUE) +
...
us_p <- ggplot(gbr) +
  geom_col(aes(x = REPORT_YEAR, y = (U_SCAR/sum(U_SCAR, na.rm = TRUE))), na.rm = TRUE) +
...
ggplot(gbr) +
  geom_jitter(aes(x = REPORT_YEAR, y = AN, col = Site_ID)) +
  theme(legend.position = "none")
...
# Examine all by site:
ggplot(gbr) +
  geom_jitter(aes(x = REPORT_YEAR, y = WHITESYNDROME, col = Site_ID)) +
  theme(legend.position = "none")
# unable to discern anything of note here - will need to examine sites more closely
WSSub <- subset(gbr, gbr$WHITESYNDROME > 0)
print(unique(WSSub$Site_ID)) # 297 sites
print(unique(WSSub$FULLREEF_ID)) # 101 reefs

ggplot(gbr) +
  geom_jitter(aes(x = REPORT_YEAR, y = SEB_DISEASE, col = Site_ID)) +
  theme(legend.position = "none")

ggplot(gbr) +
  geom_jitter(aes(x = REPORT_YEAR, y = BROWNB_DISEASE, col = Site_ID)) +
  theme(legend.position = "none")

ggplot(gbr) +
  geom_jitter(aes(x = REPORT_YEAR, y = BLACKBAND, col = Site_ID)) +
  theme(legend.position = "none")

ggplot(gbr) +
  geom_jitter(aes(x = REPORT_YEAR, y = U_SCAR, col = Site_ID)) +
  theme(legend.position = "none")
